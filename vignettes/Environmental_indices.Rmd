...
Understanding how different crop varieties respond to environmental variation is a valuable approach to provide recommendations based on genotype x environment (GxE) interactions for climate adaptation. **ClimMobTools** provides the toolkit to compute environmental variables that serves as input for the ranking analysis with explanatory variables using  [Plackett-Luce](https://hturner.github.io/PlackettLuce/)[@Turner2018] or [Bradley-Terry](https://cran.r-project.org/web/packages/psychotree/index.html)[@Wickelmaier2018] model. 
...
```{r temperature, message=FALSE, eval=TRAVIS}
...
```{r degreedays, message=FALSE, eval=TRAVIS}
...
```{r temp_gdd, message=FALSE, eval=TRAVIS}
temp <- temperature(data[c("lon","lat")], 
                    day.one = data["plantingdate"], 
                    span = gdd)
...
```{r rain, message=FALSE, eval=TRAVIS}
...
```{r rain2, eval=FALSE, message=FALSE, eval=TRAVIS}
rain <- rainfall(data[c("lon","lat")], 
                 day.one = data["plantingdate"], 
                 span = gdd,
                 days.before = 15)
...
```{r eto, message=FALSE, eval=TRAVIS}
